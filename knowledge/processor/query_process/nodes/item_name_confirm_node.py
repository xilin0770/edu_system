...
from knowledge.prompts.query import ITEM_NAME_EXTRACT_TEMPLATE
...
class ItemNameAligner():
    ...
    def match_align_filter(self, item_names:list[str]):
    ...
    def _match_vector(self, item_names:list[str]) -> list[dict[str,any]]:
        ...
        # 4. 嵌入item_name过去稠密、稀疏向量
        hybrid_embedding_result = generate_hybrid_embedding(item_names, embedding_model)

        # 5. 遍历LLM提取的每一个item_name,在milvus中进行向量搜索
        for index, item_name in enumerate(item_names):
            # 5.1 从混合向量结果中获取当前item_name的稠密、稀疏向量
...
class ItemNameExtractor:
    ...
    询问场景:(单级询问)“请问RS12-万用表如何测量电阻？” --LLM--> 商品名(item_name): 可能是[RS12-万用表,万用表测量电阻(假的但是有可能会进入confirm中去,比如:我的miluvs中刚好有一个item_name和他匹配)]
            (多级询问)“请问RS12-万用表和RS13-万用表如何分别测量电阻” --LLM--> 商品名(item_name): 可能是[RS12-万用表, RS13-万用表] --- confirm[RS12-万用表, RS13-万用表]
            (多级询问)“请问RS12-万用表和RS13-万用表如何分别测量电阻” --LLM--> 商品名(item_name): 可能是[RS12-万用表, RS13-万用表, RSDDD-万用表] --- confirm[RS12-万用表, RS13-万用表, RSDDD-万用表(误判的使用差分过滤掉)]
    """
    ...
    def extract_item_name(self, original_query: str):
        ...
        # 2. 定义提示词(用户级别)
        human_prompy = ITEM_NAME_EXTRACT_TEMPLATE.format(
            history_text = history if history else "暂无上下文",
            query = original_query
        )
        system_peompt = "你是一个专业的客服助手，擅长理解用户意图和提取关键信息。"
    ...
    def _clean_parse(self, llm_response: str) -> dict[str, any]:
        ...
        try:
            ...
            if not isinstance(rwa_item_names, list):
                ...
                clean_item_names = [rwa_item_name for rwa_item_name in rwa_item_names if rwa_item_name.strip()]
