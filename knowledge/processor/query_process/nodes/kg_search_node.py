...
class _Neo4jGraphReader:
    ...
    def _execute_seed_nodes(self, session, chinese_concept: str, entity_name: str, _kg_max_seed_candidates: int) -> List[
        ...
        # 1.精确查询
        # 使用 session.execute_read() 方法安全地执行只读查询，这是 Neo4j 推荐的最佳实践，确保查询在适当的事务上下文中运行
    ...
    def find_one_hop_relations(self, seed_nodes: List[EntitySeedNode]) -> List[OneHopRelation]:
        ...
        注意:    1.去重（不允许同一条边出现多次）只能出现一次。
                2.图谱中存储的节点和关系结构是什么 查询的时候一定要和存储的我结构保证一致
                3.邻居节点可以是你在一跳范围内指向的节点也可以别人在一跳范围内指向你的节点
    ...
    def collect_node_weight(self, seed_nodes: List[EntitySeedNode], one_hop_relations: List[OneHopRelation]) -> \
        ...
        if not one_hop_relations:
            return []
        # 存放所有节点（种子节点和邻居节点的权重）
        weight_map: list[tuple[str, str], float] = {}
        seen = set()
        # 3. 遍历所有的种子节点
        for seed_node in seed_nodes:
            # 3.1 获取item_name
...
class KnowledgeGraphSearchNode(BaseNode):
    ...
    def _run_pipeline(self, validated_query: str, validated_entity_names: List[str]) -> Dict[str, Any]:
        ...
        neo4g_graph_reader = _Neo4jGraphReader( database=self.config.neo4j_database, # neo4j
                                                ...
                                                kg_max_triples_per_seed=self.config.kg_max_triples_per_seed, # 50
